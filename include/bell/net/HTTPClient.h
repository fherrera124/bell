...
using HTTPValueHeader = std::pair<std::string, std::string>;

// Type definition for a list of HTTP headers
using HTTPHeaders = std::vector<HTTPValueHeader>;
...
class HTTPResponse {
 public:
  explicit HTTPResponse(std::unique_ptr<bell::net::SocketStream> socketStream)
   ...
   */
  HTTPHeaders getAllHeaders() const;
   ...
   */
  std::unique_ptr<bell::net::SocketStream> getStream();
...
class HTTPConnection {
 public:
  explicit HTTPConnection(const std::string& url,
   ...
   */
  void sendRequest(const std::string& method, const HTTPHeaders& extraHeaders,
   ...
   *
   * @return std::unique_ptr<HTTPResponse> Pointer to the response object
   */
 ...
 * @return std::unique_ptr<HTTPResponse> Pointer to the response object
 */
inline std::unique_ptr<HTTPResponse> get(const std::string& url,
                                         const HTTPHeaders& headers = {},
                                         int timeoutMs = httpOperationTimeout) {
  auto connection = std::make_unique<HTTPConnection>(url, timeoutMs);
 ...
 * @return std::unique_ptr<HTTPResponse> Pointer to the response object
 */
inline std::unique_ptr<HTTPResponse> postRawPtr(
 ...
 * @return std::unique_ptr<HTTPResponse> Pointer to the response object
 */
inline std::unique_ptr<HTTPResponse> post(
    const std::string& url, const HTTPHeaders& headers = {},
